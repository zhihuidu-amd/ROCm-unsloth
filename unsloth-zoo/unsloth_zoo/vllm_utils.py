...
def _get_vllm_state_dict(llm, return_state_dict = False, config = None, is_vision_model = False):
    ...
    # AMD ROCm: SM architecture concept does not apply to gfx9xx
...
def load_vllm(
    ...
    # AMD ROCm: FlashInfer requires CUDA nvcc compiler — not available on ROCm.
    # On AMD, skip FlashInfer; vLLM uses built-in paged attention (works fine).
    if is_hip() and importlib.util.find_spec("flashinfer"):
        ...
        print("Unsloth: FlashInfer skipped on AMD ROCm (requires CUDA nvcc). vLLM will use built-in attention.")
    elif not is_hip() and \
